...
static struct i2c_board_info __initdata kd_lens_dev={ I2C_BOARD_INFO("OV8830AF", 0x19)};
...
static int s4OV8830AF_WriteReg(u16 a_u2Data)
{
    ...
    //OV8830AFDB("[OV8830AF] g_sr %d, write %d \n", g_sr, a_u2Data);
    g_pstOV8830AF_I2Cclient->ext_flag |= I2C_A_FILTER_MSG;
...
inline static int getOV8830AFInfo(__user stOV8830AF_MotorInfo * pstMotorInfo)
{
	...
	if (g_s4OV8830AF_Opened >= 1)	{stMotorInfo.bIsMotorOpen = 1;}
...
inline static int moveOV8830AF(unsigned long a_u4Position)
{
    ...
    if (g_s4OV8830AF_Opened == 1)
    {
        ...
        ret = s4OV8830AF_ReadReg(&InitPos);
    ...
    //OV8830AFDB("[OV8830AF] move [curr] %d [target] %d\n", g_u4CurrPosition, g_u4TargetPosition);
...
{
    OV8830AFDB("[OV8830AF] OV8830AF_Open - Start\n");
    ...
    OV8830AFDB("[OV8830AF] OV8830AF_Open - End\n");
...
{
    OV8830AFDB("[OV8830AF] OV8830AF_Release - Start\n");

    if (g_s4OV8830AF_Opened)
    {
        OV8830AFDB("[OV8830AF] feee \n");
        g_sr = 5;
	    s4OV8830AF_WriteReg(200);
        msleep(10);
	    s4OV8830AF_WriteReg(100);
        msleep(10);
    ...
    OV8830AFDB("[OV8830AF] OV8830AF_Release - End\n");
...
inline static int Register_OV8830AF_CharDrv(void)
{
    ...
    OV8830AFDB("[OV8830AF] Register_OV8830AF_CharDrv - Start\n");
    ...
    OV8830AFDB("[OV8830AF] Register_OV8830AF_CharDrv - End\n");    
    return 0;
}

inline static void Unregister_OV8830AF_CharDrv(void)
{
    OV8830AFDB("[OV8830AF] Unregister_OV8830AF_CharDrv - Start\n");
    ...
    OV8830AFDB("[OV8830AF] Unregister_OV8830AF_CharDrv - End\n");    
}
...
static int OV8830AF_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    OV8830AFDB("[OV8830AF] OV8830AF_i2c_probe\n");
